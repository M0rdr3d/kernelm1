...
trait CassandraStatementsKernel {
  ...
  def createTimeStampIndex = s"""
      CREATE INDEX IF NOT EXISTS timestampindex on ${tableName}(timestamp);
   """
  ...
  def writeMessage = s"""
      INSERT INTO ${tableName} (persistence_id, partition_nr, sequence_nr, timestamp, timebucket, writer_uuid, ser_id, ser_manifest, event_manifest, event, event_json, tag1, tag2, tag3, message, used)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? , true)
    """
      ...
      val tables =
        if (config.tablesAutoCreate) {
          for {
            ...
            _ <- session.executeAsync(createTimeStampIndex)
